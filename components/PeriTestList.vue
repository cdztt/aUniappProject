	...
	<view class='ptl-win'>
		<view class='ptl-item'>
			...
			<view class='ptl-score'>
				<input :class="['ptl-score-input', {'ptl-score-input-edit': editable}]"
					type='number'
					v-if='!header'
					maxlength='10'
					:disabled='!editable'
					v-model="testListItem.score"
				/>
	...
	export default {
		...
		methods: {
			switchEditable() {
				...
				this.$emit('update:editable', this.isEditable)
			},
			hideKeyboard() {
				uni.hideKeyboard()
			},
